...
func RunUploadBlock(block []byte, savePath string, opt *UploadOptions) error {
	if opt == nil {
		opt = &UploadOptions{}
	}
	...
	// 临时写到temp目录下，随机文件名，不能重复
	tempFilePath := "./temp" + savePath
	...
	info := executor.Append(&pcsupload.UploadTaskUnit{
		LocalFileChecksum: checksum.NewLocalFileChecksum(tempFilePath, int(baidupcs.SliceMD5Size)),
	...
	log.Default().Println("上传百度网盘：", info.Id())
...
func RunUpload(localPaths []string, savePath string, opt *UploadOptions) {
	...
	for k := range localPaths {
		...
		for k3 := range walkedFiles {
			...
			log.Default().Println(subSavePath)
